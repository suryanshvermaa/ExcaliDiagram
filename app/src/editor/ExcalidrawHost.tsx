...
export function ExcalidrawHost() {
  ...
  const handleRenderArch = useCallback(
    (newElements: any[], files: Record<string, { id: string; dataURL: string; mimeType: string; created: number; lastRetrieved: number }>, sessionId: string) => {
      const api = apiRef.current;
      if (!api) return;
      ...
      const kept = (api.getSceneElements() as any[])
        .filter((el: any) => el.__aiGenerated !== sessionId)
        .map((el: any) => ({ ...el }))
